...
app.get('/name', isAuthenticated, async function(request, response, next) {
  ...
  let userid = request.query.userid;
	let data = await profile_db.get("SELECT * FROM Profile WHERE userid = ?", [userid]);
...
function gotProfile(accessToken, refreshToken, profile, done) {
    ...
    // should be key to get user out of database.
		...
		// async (profile) => { // for some reason does not execute
		// 	console.log("posting profile..");
		// 	await dbo.post_profile(profile);
		// }
		dbo.post_profile(profile);
...
passport.deserializeUser((userid, done) => {
    ...
    // as the property "user" of the "req" object. 
  	let result = dbo.get_name().then(function(result) {			
